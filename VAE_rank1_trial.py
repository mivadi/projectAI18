...
class VAE(nn.Module):
    ...
    def encode(self, x):
        ...
        if self.method != 'Gumbel':
            ...
            if self.rank1:
                parameter3 = self.encoder_activation(self.hidden2parameter3(x))
                return (parameter1, parameter2, parameter3)
            else:
                return (parameter1, parameter2)
        else:
            return self.encoder_activation(parameter1)
    ...
    def reparameterize(self, parameters):
        ...
        if self.method == 'Gaussian':
            if self.rank1:
                (mean, v, w) = parameters
                epsilon = torch.FloatTensor(v.size()).normal_()
                Sigma = torch.bmm( v.unsqueeze(2), w.unsqueeze(1) )
    ...
    def log_q_z(self, z, parameters):
        ...
        if self.method == 'Gaussian':
            if self.rank1:
                ...
                z_mean = z - mean
                log_distr = -0.5 * ( log_det_cov_matrix + torch.bmm(z_mean.unsqueeze(1), torch.bmm( inverse_cov_matrix, z_mean.unsqueeze(2) ) ) ) 
            else:
                (mean, logvar) = parameters
